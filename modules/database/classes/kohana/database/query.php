...
class Kohana_Database_Query {
	...
	public function execute($db = 'default')
	{
		...
		if ( ! isset($_REQUEST['sql_cache']))
			$_REQUEST['sql_cache'] = array();
			
		if ($this->_type === Database::SELECT)
		{
			// Set the cache key based on the database instance name and SQL
			$cache_key = $sql;
			$result = arr::get($_REQUEST['sql_cache'], $cache_key, NULL);
			if ($result != NULL)
			{
		...
		if (isset($cache_key))
		{
			...
			$_REQUEST['sql_cache'][$cache_key] = $result_array;
			
			return new Database_Result_Cached($result_array, $sql);
		}
